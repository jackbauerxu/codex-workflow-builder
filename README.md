...
![Production 路线：设计、脚本、预览、渲染、验证](assets/readme/production-pipeline.svg)
...
├── src/                 # typed content、Root 和可复用 composition
├── scripts/             # still/MP4 产物签名检查
├── package-lock.json    # 可复现依赖
└── outputs/             # 已验证的首帧 PNG 与短 MP4 证据
```

它不是只写流程的示意：在本地或 CI 中会真实执行 typecheck、still frame、H.264 smoke render 和二进制产物检查。
...
本仓库的真实渲染证据见 [`examples/remotion-starter/outputs/`](examples/remotion-starter/outputs/)。它只证明最小样例已渲染，不代表任何用户项目的内容已经交付。
...
它使用 `minimal` 预设、1080×1920、30fps、60 帧和内置 typed content，不依赖外部素材；`verify:artifacts` 会检查 PNG/MP4 的真实签名。CI 会重复这条路径并上传短期渲染证据。
...
- `SKILL.md`：Codex 实际读取的精简 Skill 主体；详细工作流按需加载 `references/workflow-patterns.md` 和 `references/video-workflows.md`。
...
- `remotion/`：交接给真实 Remotion 工程或子 Skill 的生产合同。
- `examples/remotion-starter/`：可真实渲染的最小 Remotion 工程，包含锁文件、首帧、MP4 和产物检查。
- `tests/`：检查生产资料、Skill 路由、仓库完整性和 metadata 是否同时存在。
...
- 边界模糊的内容生产请求
- 可运行 Remotion starter 的 typecheck、still frame、MP4 smoke render 和 PNG/MP4 签名检查
- Skill metadata、License、CI、贡献指南、Change Log 和主 Skill 拆分边界
...
cd examples/remotion-starter && npm ci && npm run typecheck && npm run render:still && npm run render:smoke && npm run verify:artifacts
```
...
本仓库自有内容使用 [MIT License](LICENSE)。Seedance 方法的来源与致谢见 `prompts/generative-video.md` 和 `README.md`；第三方参考视频、图片、字体、音乐和素材仍需由使用者自行确认授权。
